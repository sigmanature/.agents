...
Options:
  ...
  -p, --package <pkg.name>  package to constrain monkey (recommended)
...
if [[ "$GLOBAL_MODE" -eq 0 && -z "$PKG" ]]; then
...
if [[ "$GLOBAL_MODE" -eq 0 ]]; then
  "${ADB[@]}" shell dumpsys meminfo "$PKG" > "$OUT_DIR/dumpsys_meminfo_start.txt" || true
else
