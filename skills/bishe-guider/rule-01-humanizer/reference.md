# 规则 1：像个人类一样写作

## 适用范围

本规则适用于中文学术论文的全部文本润色与审查，目标是将 AI 生成的刻板痕迹去除，使论文读起来像一位严谨的科研工作者亲笔所写。

## 核心原则

1. **识别 AI 模式** — 扫描并标记下文列出的全部可疑模式
2. **重写问题段落** — 用自然、具体的学术表达替换 AI 痕迹
3. **保留原意** — 不改变核心论点、数据和结论
4. **维持语体** — 保持中文学术论文的正式、客观、严谨风格
5. **注入人味** — 不是简单删除坏模式，而是让行文有真实研究者的思考节奏

---

## AI 写作痕迹检测清单

### 一、内容模式（Content Patterns）

#### 1. 过度强调意义、 legacy、宏观趋势

**警示词：** 标志着、见证了、承载着、凸显了、反映了更广泛的、象征着、为……奠定基础、开启了、塑造了、代表了……的转变、关键的转折点、不断演进的格局、留下了不可磨灭的印记

**问题：** AI 喜欢把任意细节拔高到宏大叙事。

**修改示例：**
- ❌ 本研究的提出标志着医学智能体领域进入了一个新的发展阶段，为后续研究奠定了基础。
- ✅ 本研究聚焦于医学智能体的反馈验证问题，提出了一种新的迭代优化方法。

#### 2. 宣传性、广告式语言

**警示词：** 得天独厚的、 vibrant（充满活力的）、丰富的（比喻义）、深刻的、提升其、展示、体现、致力于、自然美景、坐落于、心脏地带、开创性的、享誉、 breathtaking、必访、 stunning

**修改示例：**
- ❌ 本系统凭借得天独厚的架构优势，展示了卓越的性能表现。
- ✅ 本系统在标准测试集上取得了 92.3% 的准确率，优于现有方法 4.7 个百分点。

#### 3.  superficial -ing 分析

**警示词：** 凸显了……、确保了……、反映了……、象征着……、为……做出贡献、培养/促进……、涵盖……、展示……

**问题：** AI 喜欢在句尾附加现在分词短语来伪造深度。

**修改示例：**
- ❌ 实验结果表明该方法有效，凸显了其在医学诊断中的潜在应用价值。
- ✅ 实验结果表明该方法在医学诊断任务中的准确率达到 91.2%。

#### 4. 模糊归因与 weasel words

**警示词：** 行业报告指出、观察者认为、专家认为、一些批评者认为、多个来源（实际引用很少）

**修改示例：**
- ❌ 专家认为，该方向具有重要的研究价值。
- ✅ Wang 等 [1] 指出，该方向具有重要的研究价值。

#### 5. 套路化的"挑战与展望"段落

**警示词：** 尽管其……面临若干挑战……、尽管存在这些挑战……、挑战与遗产、未来展望

**修改示例：**
- ❌ 尽管该方法取得了一定成果，但仍面临计算复杂度高、数据依赖性强等挑战。展望未来，随着技术的不断进步，这些问题有望得到解决。
- ✅ 该方法的主要局限在于计算复杂度较高（单次推理耗时约 3.2 秒），且对标注数据的质量较为敏感。

### 二、语言与语法模式

#### 6. AI 高频词汇

**高频 AI 词：** 此外、与……对齐、 crucial、深入探讨、强调、持久的、增强、促进、 garner（获得）、 highlight（动词）、 interplay（相互作用）、 intricate（复杂的）、关键的、 landscape（格局）、 pivotal（关键的）、展示、 tapestry（织锦）、 testament（证明）、 underscore（强调）、 valuable、 vibrant

**修改示例：**
- ❌ 此外，该方法通过复杂的 interplay 提升了系统的整体 performance landscape。
- ✅ 该方法通过协调多个子模块的交互关系，提升了系统整体性能。

#### 7. 避免使用 "是/为"（系动词回避）

**警示词：** 作为、标志着、代表着 [一个]、拥有/具备/提供 [一个]

**修改示例：**
- ❌ 本系统作为一个高效的医学问答平台，具备快速响应能力。
- ✅ 本系统是高效的医学问答平台，响应速度可达 200ms。

#### 8. 否定性平行结构

**问题：** "不仅……而且……"、"这不只是……，这是……" 等结构过度使用。

**修改示例：**
- ❌ 该模块不仅提升了准确率，而且增强了系统的鲁棒性。
- ✅ 该模块将准确率从 85% 提升至 91%，同时将噪声容忍度提高了 12%。

#### 9. 三一律过度使用

**问题：** AI 喜欢把内容硬凑成三点，以显得全面。

**修改示例：**
- ❌ 本研究的主要贡献包括：提出了新模型、设计了新算法、验证了新假设。
- ✅ 本研究提出了基于反馈验证的医学智能体优化框架，并在三个标准数据集上验证了其有效性。

#### 10. 同义词循环（Elegant Variation）

**问题：** AI 的重复惩罚机制导致过度替换同义词。

**修改示例：**
- ❌ 该方法具有高效性。该算法具备快速性。本方案拥有迅捷性。
- ✅ 该方法计算效率高，单次推理仅需 0.3 秒。

#### 11. 虚假范围

**问题：** "从 X 到 Y" 结构中 X 和 Y 不在同一有意义的尺度上。

**修改示例：**
- ❌ 本研究从理论分析到实际应用，从算法设计到系统实现，全面探索了医学智能体的优化路径。
- ✅ 本研究在理论分析的基础上，设计了优化算法并完成了系统实现。

### 三、风格模式

#### 12. 破折号过度使用

**问题：** AI 比人类更频繁使用 em dash（——），模仿"有力"的营销写作。

**修改示例：**
- ❌ 该方法——虽然简单——但效果显著——值得进一步研究。
- ✅ 该方法虽然简单，但效果显著，值得进一步研究。

#### 13. 粗体过度使用

**问题：** AI 机械地给短语加粗。

**修改示例：**
- ❌ 本系统融合了 **Transformer 架构**、**注意力机制** 和 **预训练技术**。
- ✅ 本系统融合了 Transformer 架构、注意力机制和预训练技术。

#### 14. 行内标题式垂直列表

**问题：** 列表项以加粗标题加冒号开头。

**修改示例：**
- ❌ - **准确率：** 系统在测试集上达到 95% 的准确率。
  - **效率：** 推理速度提升了 40%。
- ✅ 系统在测试集上的准确率达到 95%，推理速度提升了 40%。

#### 15. 标题大小写问题

**问题：** AI 把标题中所有实词首字母大写。

**修改示例：**
- ❌ ## 实验结果与分析
- ✅ ## 实验结果与分析（中文无此问题，但英文标题需注意）

#### 16. 表情符号

**问题：** AI 在标题或列表项中添加表情符号。

**绝对禁止在学术论文中使用任何表情符号。**

### 四、填充与模糊表达

#### 17. 填充短语

| 问题表达 | 修改为 |
|---------|--------|
| 为了实现这一目标 | 为此 |
| 由于下雨这一事实 | 因为下雨 |
| 在此时刻 | 现在 |
| 如果你需要帮助的事件发生 | 如果你需要帮助 |
| 系统具有处理……的能力 | 系统可以处理…… |
| 值得注意的是，数据显示 | 数据显示 |
| 通过……的方式 | 通过…… |

#### 18. 过度模糊化

**修改示例：**
- ❌ 该策略可能对结果产生一定影响。
- ✅ 该策略将 F1 分数提升了 2.3 个百分点（p < 0.05）。

#### 19. 套路化的正面结尾

**修改示例：**
- ❌ 展望未来，该方法具有广阔的应用前景，必将为医学智能体的发展注入新的活力。
- ✅ 未来工作将探索该方法在多模态医学数据上的扩展。

---

## 中文学术论文专属检查项

| 检查项 | 问题示例 | 修改示例 |
|--------|---------|---------|
| "本研究"/"本文章"/"我们" | 本研究提出了一种方法 | 本论文提出了一种方法 |
| 夸张词汇 | 首次提出、显著提升、重大突破 | 提出了一种、提升了 X%、取得了进展 |
| 口语化表达 | 效果不错、很有意思 | 效果显著、具有理论价值 |
| 长句堆砌 | 一句话超过 50 字 | 拆分为 2-3 个短句 |
| 被动语态滥用 | 被用于、被认为 | 可用于、研究者认为 |
| 空泛修饰 | 非常、十分、极其 | 删除或改为具体数据 |

### 源码片段中的中文注释

对于计算机、操作系统、内核等方向的论文，源码片段也会影响机器审稿对文本风格的判断。正文解释可以使用中文，但代码块内部应尽量保持真实源码摘录的形态：

1. 代码块内不保留中文解释性注释、临时调试说明和 TODO。
2. 若需要保留源码位置，只使用英文 source 锚点，例如 `/* Source: fs/f2fs/data.c:5086-5137, f2fs_set_iomap */`。
3. 行尾中文注释应删除注释部分，保留前面的可执行语句。
4. 脚本中的中文字符串字面量、论文伪代码中的“输入/输出”、正文解释段不按“代码注释”处理。

### 长句拆分的边界

长句拆分应按论证语义进行，而不是按逗号、分号或固定字数机械切断。尤其在中文学术论文中，引用句和技术说明句常包含英文论文题名、函数名、路径名或参数名，盲目按标点拆分会产生“指出。”、“从而……”、“以及……”这类不完整句。

处理长句时优先采用以下方式：

1. 将“作者/文档指出 + 具体结论 + 本文承接”拆成 2 到 3 个完整短句。
2. 将“对象说明 + 机制说明 + 后续影响”拆成独立句。
3. 保留必要的论文题名、函数名和路径名，不为了压低字数破坏技术事实。
4. 拆分后必须抽样朗读检查，确认没有独立的承接词、半句和机械断句。

---

## 处理流程

1. 通读待审查文本
2. 标记所有符合上述模式的表达
3. 逐句重写，保留核心语义
4. 检查修改后的文本：
   - 朗读是否自然
   - 句式是否有变化
   - 是否用具体数据替代模糊声称
   - 是否符合学术语体
5. 输出修改建议表格（详见 SKILL.md 输出格式要求）
