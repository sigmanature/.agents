...
#   /sys/fs/f2fs/<id>/dbg_wcf_verity_only
#
# Usage:
#   ./set_f2fs_wcf_filters.sh --fs-id userdata --ino1 123 --ino2 456 --verity-only 1
...
Options:
  ...
  --verity-only <0|1> require fsverity_active || verity_in_progress for logs
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ino1) INO1="$2"; shift 2;;
    --ino2) INO2="$2"; shift 2;;
    --verity-only) VERITY_ONLY="$2"; shift 2;;
...
P3="$SYS_BASE/dbg_wcf_verity_only"
...
if [[ -z "${VERITY_ONLY:-}" ]]; then VERITY_ONLY="0"; fi

echo "[+] Setting f2fs write_cache_folios filters:"
echo "    fs-id=$FS_ID ino1=$INO1 ino2=$INO2 verity_only=$VERITY_ONLY"

"${ADB[@]}" shell su -c "sh -c 'echo $INO1 > $P1; echo $INO2 > $P2; echo $VERITY_ONLY > $P3; cat $P1; cat $P2; cat $P3'" || {
